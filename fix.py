...
filepath = 'e:/pklnew/src/pages/admin/OperationsPage.tsx'

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# Chunk 1: Replace placeholder with RecentActivityFeed
from1_pattern = r'<div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-center">\s*<Clock size=\{32\} className="mx-auto text-slate-300 mb-3" />\s*<p className="text-sm text-slate-500 font-medium">Buka <b className="text-slate-700">Riwayat Operasional</b> untuk melihat detail histori dan filter data secara lengkap.</p>\s*</div>'

to1 = """<RecentActivityFeed />
...
if re.search(from1_pattern, content):
    content = re.sub(from1_pattern, to1, content)
    print('Chunk 1 replaced')
else:
...
if re.search(from2_pattern, content):
    content = re.sub(from2_pattern, to2, content)
    print('Chunk 2 replaced')
else:
